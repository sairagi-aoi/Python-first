...
def write_summary_csv(results, output_path='summary.csv'):
      """
      集計結果をCSVファイルに書き出す。
...
def write_summary_txt(results, output_path='summary.txt'):
      """
      集計結果をテキストファイルに書き出す。
      ...
      with open(output_path, 'w', encoding='utf-8') as f:
          ...
          f.write(f"{'日付':<15} {'飼い主平均体調':>15} {'飼い猫平均体調':>15}\n")
          f.write("-" * 70 + "\n")

          for row in results:
              date = row['date']
              avg_owner = row['avg_owner_condition']
              avg_cat = row['avg_cat_condition']
              f.write(f"{date:<15} {avg_owner:>15.2f} {avg_cat:>15.2f}\n")

          f.write("\n" + "=" * 70 + "\n")
          f.write(f"総データ数: {len(results)} 日分\n")

          # 全体の平均も計算
          overall_owner_avg = sum(r['avg_owner_condition'] for r in results) / len(results)
          overall_cat_avg = sum(r['avg_cat_condition'] for r in results) / len(results)

          f.write(f"全期間の飼い主平均体調: {overall_owner_avg:.2f}\n")
          f.write(f"全期間の飼い猫平均体調: {overall_cat_avg:.2f}\n")
          f.write("=" * 70 + "\n")
...
def main():
      ...
      # コマンドライン引数のチェック
      if len(sys.argv) < 2:
          print("使用方法: python tai_coude.py input.csv")
          sys.exit(1)
      ...
      print(f"集計が完了しました。")
      print(f"  - CSV形式: summary.csv")
      print(f"  - テキスト形式: summary.txt")
